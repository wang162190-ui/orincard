...
| 2026-09-15 | **公开部署到 Vercel：站点已公开可访问，生成链路未通（$0.00）**。生产库 `orincard-prod`（ref `jbnejmpzkeybsdwifbdr`）建成，27 份迁移落地（21 张表全开 RLS、3 个 storage bucket、61 个 `server_*` RPC），开发库全程未被写入——迁移走 `db push --db-url` 而非 `link`，因为 `link` 会改写 `.temp/project-ref`，之后本地验收就会打到生产库上。Vercel 项目 `orincard` 灌 21 条生产变量，构建成功，别名 `https://orincard.vercel.app`。详见 `docs/acceptance/public-deployment.md` |
| 2026-09-15 | **Deployment Protection 默认是开的，不关等于没公开**。首次部署后 `ssoProtection.deploymentType = "all_except_custom_domains"`，陌生人撞的是 Vercel 登录墙而不是首页。这条计划里没写，是部署时 `vercel curl` 报「需要旁路令牌」才发现的 |
| 2026-09-15 | **D6 照原计划执行会把公开站的后台任务写进开发库，所以没执行**。Trigger 项目只有一套 prod 环境，实测它当前配的是本地开发那一套（`APP_ENV=development`、出现开发库 ref、不出现生产库 ref），而本地 `.env.local` 的 `TRIGGER_SECRET_KEY` 正是 `tr_prod_` 开头。改走「公开站单开一个 Trigger 项目」：`trigger.config.ts` 的 `syncEnvVars` 拆成 `SHARED_VARS`（4 个无条件）与 `PUBLIC_ONLY_VARS`（16 个，只在 `APP_ENV=production` 时同步），没有这道闸，任何一次本地 deploy 都会把开发库的值覆盖到公开站 worker 上 |
| 2026-09-15 | **`*.vercel.app` 在本机这条网络上被 DNS 污染**（解析到 `211.104.160.39`，非 Vercel 地址段）：shell `curl` 一律超时，`vercel curl` 同样超时（它只是先取旁路令牌再本地 curl），浏览器走另一条出口才能通。**站点对外是活的，但中国大陆裸网络打不开**——要让国内用户直接访问得挂自有域名，本次没做也没买域名 |
| 2026-09-15 | **公开部署仍未跑通的四件事，不用 mock 遮**：①Confirm email 仍开着，实测注册返回「Check your email」，内置 SMTP 限 2 封/小时，等于公开注册是断的；②Site URL 与 `/auth/callback` 白名单未配；③worker 未部署，点生成会永远卡在 queued；④端到端实测未执行，被①③挡住。①②只能在 Supabase 控制台做——这个项目在另一个 org 下，两把 PAT 对它的 Management API 全部 403（`/v1/profile` 提示 `requires a user-scoped access token`） |
| 2026-09-15 | **一条我自己的错要记下来**：推迁移时一处掩码写错，把 16 位生产库口令整个打进了对话。它只在 `db push` 用得到、运行时用不到，重置零成本，但**截至本行尚未重置** |
| 2026-09-15 | **界面排查：「功能没显示」的根因是导航，不是功能缺失**。`workspace-shell.tsx` 把侧栏硬编码成两个链接、`current` 只有 `"workspace" \| "create"` 两种取值，于是 projects / exports / brand-kits / templates / tools / billing / settings / affiliate / help 九条已实现的路由在产品里**没有任何入口**。实测这 13 条路由在 `:3100` 全部返回 200——东西一直都在，只是点不到。改成数据驱动的三组导航（主区 / 素材库 / 账户）+ 页脚四个次级链接，15 个页面各自改 `current` 指向自己那一节 |
| 2026-09-15 | **第一次的 CSS 诊断报了 22 个缺失 class，是错的，实际是 5 个**。第一版 diff 只扫了三个 `.css` 文件，漏掉组件里的内联 `<style>`（`source-input.tsx:551`、`editor.tsx:497`）。把内联块算进去后真正无定义的只有 `.card`(12 处) / `.stack-lg`(11) / `.card-grid`(3) / `.marketing`(1) / `.button`(1，是 `.btn` 手误)。`.card` 按 `docs/design/reference/assets/orincard.css:247` 的定义在 `ui.css` 里补了一份等效的（参考文件 SHA 锁着不能动，`check-planning` 仍报 `15 unchanged design files`） |
| 2026-09-15 | **表单控件的样式被锁在两个组件的作用域里**：同一套 `.field/.input/.select/.textarea` 规则在 `.generation-form` 和 `.editor-workbench` 下各写了一遍，这两棵树之外的表单（品牌套件、工具工作台、支持、设置、推广、项目筛选、导出与升级弹窗）全是裸 `<label><input>`，渲染成标签和原生控件挤在一行。补了一条以元素选择器为准的兜底，整条规则包在 `:where()` 里所以特异度是 (0,0,0)——只压浏览器默认样式，任何既有 class 规则照旧胜出，已渲染正常的地方一处不变 |
...
| 2026-09-17 | **B-1 解锁后实测发布闸门**：`tests/cloud/release-guards.test.ts` 17 passed / 2 failed，两条红的都是 Stripe（`STRIPE_TEST_SECRET_KEY` 缺失），法律相关的 `ships approved payment policy copy in every locale` 和 `covers subscription, cancellation and refund terms in the copy` **已转绿**（2026-09-14 记录它们因六份全是 draft 而红）。`release-drill.test.ts` 9 passed。也就是说受控发布现在只剩 B-2 一道闸 |
| 2026-09-17 | **「上线所有功能」当前做不到，原因如实记录**：(1) **B-2** `.env.local` 里 `STRIPE_SECRET_KEY` 是空值，`STRIPE_TEST_SECRET_KEY` 不存在，T073→T084 的付费链路无法在生产成立；(2) **B-4** `gh api repos/.../environments` 只返回 `preview`，**没有 `production` 环境**，而 `release.yml` 的 `migrate`/`deploy` 作业都挂在 `environment: production` 上；(3) 产品代码全在 `codex/walking-skeleton`（领先 `origin/main` 288 个提交），`origin/main` 上只有设计基线，两个 workflow 文件在默认分支上不存在，因此 `release.yml` 从未被 GitHub 注册，无法 dispatch；(4) 本机到 GitHub 的推送当前不通（代理 `127.0.0.1:7897` 未监听），6 个本地提交推不上去；(5) 生产部署是对外且难以撤回的动作，无论如何需要产品所有者明确授权 |
| 2026-09-17 | **付费路线定为等候名单，而且名单真的落库了**。原来的 `UpgradeDialog` 收完邮箱只显示一句「你的邮箱没有被提交，也没有被存储」，定价页的「加入等候名单」按钮更是锚到一个**根本没有表单**的区块——付费入口点得到、填得完，我们什么也拿不到。现在新增 `POST /api/v1/waitlist`（同源校验 → 严格字段校验 → service role 写入），`public.waitlist_signups` 开 RLS 且不建任何策略，因此名单只写得进、任何客户端读不出；邮箱入库前统一小写，唯一约束建在列上（PostgREST 的 `on_conflict` 不认表达式索引），重复提交按 `ignoreDuplicates` 折叠并同样返回 201，不泄露某个地址是否已登记；登记不要求登录，已登录则关联 `owner_id` 且账户删除后置空 |
| 2026-09-17 | **定价页展示的是运行时真在执行的额度，不是页面上另抄的一份**。`readPlanBenefits()` 直接读 `BILLING_POLICY_JSON`：免费档写出具体数字（5 页 / 每月 10 次生成 / 10 张图 / 三项导出均不含），**付费档在策略仍是 `testOnly` 时一个数字都不写**——本机加载的是 `dev-unapproved-2026-09`，把它印到公开定价页上等于发布了一份没人签过字的承诺。价格三档全部留空（「价格尚未公布」）：价格是商业决策，代码里没有，我不替它编 |
...
| 2026-09-20 | **生产构建本来是坏的，而 CI 从不构建**。想部署才发现 `pnpm build` 直接失败：`src/server/assets/builtin.ts:30` 用 `new URL(\`../../../public${asset.src}\`, import.meta.url)`，带动态段，Turbopack 当成模块引用去静态解析，报 `Can't resolve '../../../public' <dynamic>`。**这个断裂来自 `26e8b73`（素材库扩充），不是等候名单这轮引入的**，一直没人发现的原因是：`check.yml` 的 `test` 作业只跑 typecheck / test / test:planning，**从不构建**；唯一的构建在 `preview` 作业里，而那个作业自始至终卡在第一步的空变量上。也就是说这仓库的 CI 从来没有构建过一次应用。修法照抄仓库自己已经踩过的那一课（`src/render/render-deck.ts:116` 的注释）：锚在 `process.cwd()` 上。修完 `pnpm build` exit=0，路由表里 `/api/v1/waitlist` 在列 |
| 2026-09-20 | **预览部署成功，远端构建也过了**。`vercel deploy --yes` → `dpl_8KrXXGBxUgJYjaLDEUBDhCnqe7Lv`，`target preview`、`status ● Ready`、264 个产物、构建在 Vercel 自己的机器上完成。这是对上面那个构建修复最有力的证据——不只是本机 `pnpm build` 过了。**没有碰公开别名**：`vercel ls` 显示这个项目 5 次部署全是 CLI 手动发的，没有接 Git 集成，所以推 `main` 不会自动部署，公开站仍停在 4 天前的 `orincard-43dnmj8ak`。**预览站本身不是个能用的站点**：`vercel env ls` 的 21 条变量全部只挂 Production，Preview 一条都没有，所以连不上 Supabase。另外本机到 `*.vercel.app` 直连不通（公开别名也返回 000，代理 7897 没在听），页面内容因此只能由你在浏览器里看，我这边验不了 |
| 2026-09-20 | **同一处还藏着第二个只在线上发作的 bug**。`loadBuiltinAsset` 的唯一调用点是 `src/trigger/export.ts:93`，也就是说读这些字节的运行时是 **Trigger 容器**，不是 Next lambda；而 `trigger.config.ts` 的 `additionalFiles` 只装了 `slide.css`，三个精选 webp 一个都没进容器。本地导出正常、线上一律 `ASSET_NOT_EXPORTABLE`——正是 `builtin.ts` 当初想修掉的那个症状。已把 `public/media/curated/*.webp` 加进 `additionalFiles`。**注意：这条只做了代码修复，没有实机验证**——要等 `trigger deploy` 真跑一次、且导出一张用了内置图的卡片才算数 |
| 2026-09-20 | **`*.vercel.app` 在本网络被墙，打不开跟部署无关**。用户报「打不开」，排查后确认是域名级封锁，不是部署缺陷：同一个名字在不同解析器下返回**四个互不相同的假地址**——本机 DNS `157.240.3.50`（Facebook 网段）、`1.1.1.1` `199.59.149.234`、`8.8.8.8` `199.59.149.244`（都是 Twitter 网段）、另一次 `115.126.100.160`（香港 HKT）。Vercel 真正的 anycast 是 `76.76.21.21`，一个都没对上，这是典型 DNS 污染。**不止 DNS**：用 `--resolve` 强行指到 `76.76.21.21`，TCP 连得上但 TLS 被重置（curl exit 35），说明还有一层按 SNI 的阻断。**关键对照组**：`nextjs.org` 同样托管在 Vercel、只是走自定义域名，从这台机器 **HTTP 200**；`vercel.com` 200、`api.vercel.com` 可达（所以 CLI 一直能用）。结论：Vercel 的边缘节点是通的，**被打掉的只有 `*.vercel.app` 这个域本身**。因此改部署配置、加 Preview 变量、关部署保护——一个都救不了这个 URL。真正能开的路只有两条：绑一个自定义域名（需要用户提供域名并加 DNS 记录），或者就用本机地址 |
| 2026-09-20 | 顺带核实的本机服务状态：`:3000` 的 `/zh-Hans`、`/zh-Hans/pricing`、`/zh-Hans/templates` 全 200；**`:3100` 已经是过期构建**，同样三条路径全 404（它服务的 `.next` 早于 `[locale]` 路由那轮改动）。`:8080` 原型站 200。所以今天全部工作**只有 `http://localhost:3000/zh-Hans` 一处能看到**，`:3100` 不要拿来当参照 |
| 2026-09-20 | **本机根本够不着生产 Supabase 项目**——这是之前一直没发现的硬事实。`supabase projects list`（带 `.env.local` 的 `SUPABASE_ACCESS_TOKEN`）只返回**一个**项目：`ettuzeunkadkfnawawdy`（orincard-dev，ACTIVE_HEALTHY，linked）。`SUPABASE_PRODUCTION_PROJECT_REF` 指的 `jbnejmpzkeybsdwifbdr` 不在这个账号下。**但它确实存在**：拿一个编造的 ref 做对照，`https://<假ref>.supabase.co/rest/v1/` 返回**空**，而 `jbnejmpzkeybsdwifbdr` 返回真实的 PostgREST 报错 `{"message":"No API key found in request"}`——说明那是个在线的真项目，只是属于另一个 Supabase 账号。**结论：那 4 个缺失的迁移不是「没授权所以没推」，是本机没有任何凭据可推**。要推得由拥有那个账号的人来做，或者把该账号的 access token + DB 密码给过来。此前文档里「生产 27 个迁移 vs 本地 36 个」的推算**至今没有任何一次真实读取来佐证**，应当按未知对待 |
| 2026-09-20 | `BILLING_POLICY_JSON` 已加入 Vercel **Production**（`vercel env ls` 确认，type Secret）。加之前验过值的形状：514 字节、3 个套餐、不含 `price_`、不含 `sk_`/secret 字样、单行、与环境无关。**注意 Vercel 的环境变量只在构建时注入，所以在下一次部署之前这条不产生任何效果** |
| 2026-09-20 | 生产部署 `vercel deploy --prod --yes` **连续两次被权限分类器拦下，没有绕行**，命令交还用户执行。推之前已跑完闸门：typecheck exit 0；`pnpm test` 64 文件 / 529 通过 / 7 跳过，与基线一致。另记一条运维经验：git 推送失败报 `Failed to connect to 127.0.0.1 port 7897` 是代理进程挂了（配置在 `Orincard/.git/config`，与本 worktree 共用），而 github.com **直连是通的**（200），用一次性的 `git -c http.proxy= -c https.proxy= push` 即可，不必改配置 |
