...
| B15 | 等候名单与套餐权益展示（AC-009/AC-011，2026-09-17 追加） | B14；本轮不接支付，付费路线只有等候名单，因此名单必须真的落库而不是前端提示；定价页的额度一律读运行时策略，付费档在策略未审定前不展示数字，价格全部留空 |

关键路径：B01 → B02 → B03 → B04 → B05 → B06 → B07 → B08 → B09 → B10 → B11 → B12 → B13 → B14 → B15。风险探针失败先修方案，不能继续堆页面；B04 是第一套真实可导出闭环，B12 才是全量验收。
