...
/**
 * Names the preview files; it does not draw them. `scripts/render-template-previews.mjs`
 * consumes this list and screenshots the real editor into exactly these paths. Running
 * this script alone leaves the manifest pointing at files that do not exist yet.
 */
export async function previewManifest() {
  ...
  return templates.map((template) => {
    ...
    const files = template.document.slides.map((_, index) => `/media/templates/${template.slug}-${version}-${index + 1}.webp`);
