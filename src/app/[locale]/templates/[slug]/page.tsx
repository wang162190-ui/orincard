...
export default function TemplateDetailPage({ params }: { readonly params: Promise<{ readonly slug: string }> }) {
  ...
  return <WorkspaceShell current="templates" title={template.name}><div className="stack-lg"><header><p className="eyebrow">{template.category} · {document.platform}</p><h1>{template.name}</h1><p className="lead">{template.description}</p><div className="row"><Button disabled={busy} onClick={() => void createCopy()}>{busy ? t("creating") : t("use")}</Button><Link className="btn btn-secondary" href="/templates">{t("all")}</Link></div>{notice ? <p role="alert">{notice}</p> : null}</header><section className="card stack" aria-label={t("preview")}><h2>{t("outline")}</h2>{document.slides.map((slide, index) => <article key={slide.id} data-testid="template-slide"><p className="eyebrow">{t("page", { number: index + 1, role: slide.role })}</p><h3>{slide.title}</h3>{slide.bodyBlocks.map((block, blockIndex) => <p key={blockIndex}>{block.kind === "bullets" ? block.items.join(" · ") : block.text}</p>)}{slide.cta ? <p><strong>{slide.cta}</strong></p> : null}</article>)}</section></div></WorkspaceShell>;
}
