...
async function document(slug: string, locale: string) {
  try { return await readContent("guide", slug, locale as ContentLocale); } catch (error) { if (error instanceof ContentNotFoundError) notFound(); throw error; }
}
...
export default async function GuideArticle({ params }: { readonly params: Promise<{ readonly slug: string; readonly locale: string }> }) {
  const { slug, locale } = await params;
  const content = await document(slug, locale);
  const t = await getTranslations("Guides");
  return <WorkspacePage title={t("shellTitle")}><article className="card stack-lg"><header><p className="eyebrow">{t("eyebrow")}</p><h1>{content.title}</h1><p>{content.description}</p></header>{content.blocks.map((block, index) => <Block block={block} key={`${block.kind}-${index}`} />)}</article></WorkspacePage>;
}
