...
export async function POST(request: Request, context: { readonly params: Promise<{ readonly id: string }> }) {
  ...
  try {
    ...
    const data = await createProjectLibraryService({ client: createAdminSupabaseClient(), requestHashSecret: environment.supabaseSecretKey }).archive(ownerId, id, (body as { expectedRevision?: unknown }).expectedRevision, request.headers.get("idempotency-key") ?? "");
