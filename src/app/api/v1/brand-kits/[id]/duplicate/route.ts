...
export async function POST(request: Request, context: { readonly params: Promise<{ readonly id: string }> }) {
  ...
  try {
    ...
    const kit = await createBrandService(createSupabaseBrandStore(createAdminSupabaseClient())).duplicate(await ownerId(), id, { expectedRevision: input.expectedRevision, name: input.name });
