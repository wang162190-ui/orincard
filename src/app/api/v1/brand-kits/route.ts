...
import { createBrandService, BrandServiceError, createSupabaseBrandStore } from "@/server/brands";
...
export async function GET(request: Request) {
  ...
  try {
    const service = createBrandService(createSupabaseBrandStore(createAdminSupabaseClient()));
    const kits = await service.list(await ownerId(), new URL(request.url).searchParams.get("limit") ?? undefined);
    return Response.json({ data: { kits, nextCursor: null }, requestId }, { headers: { "Cache-Control": "private, no-store" } });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const service = createBrandService(createSupabaseBrandStore(createAdminSupabaseClient()));
    const kit = await service.create(await ownerId(), body as Record<string, unknown>);
    return Response.json({ data: { kit }, requestId }, { status: 201, headers: { "Cache-Control": "private, no-store" } });
  } catch (error) { return errorResponse(error, requestId); }
}
