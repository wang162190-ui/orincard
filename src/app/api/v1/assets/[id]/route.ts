...
function failure(error: unknown, requestId: string): Response {
  const detail = error instanceof AssetLibraryError ? error : new AssetLibraryError("SERVICE_UNAVAILABLE", "This asset could not be deleted.", 503, true);
...
export async function DELETE(request: Request, context: { readonly params: Promise<{ readonly id: string }> }) {
  ...
  try {
    ...
    let ownerId: string;
    try { ownerId = (await requireVerifiedUser(user)).id; } catch { throw new AssetLibraryError("AUTH_REQUIRED", "Sign in before deleting an asset.", 401); }
    ...
    await createAssetLibraryService({ store: createSupabaseAssetLibraryStore(createAdminSupabaseClient()) }).remove(ownerId, id, body as Record<string, unknown>);
    return Response.json({ data: { assetId: id, state: "deleted" }, requestId }, { headers: { "Cache-Control": "private, no-store" } });
