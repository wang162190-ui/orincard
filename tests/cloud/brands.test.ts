...
function store(overrides: Partial<BrandStore> = {}): BrandStore {
  return { list: vi.fn().mockResolvedValue([ROW]), get: vi.fn().mockResolvedValue(ROW), create: vi.fn().mockResolvedValue(ROW), update: vi.fn().mockResolvedValue({ ...ROW, revision: 2 }), listAffectedProjects: vi.fn().mockResolvedValue([]), delete: vi.fn().mockResolvedValue(true), availableAssetIds: vi.fn().mockResolvedValue([ASSET]), ...overrides };
}
